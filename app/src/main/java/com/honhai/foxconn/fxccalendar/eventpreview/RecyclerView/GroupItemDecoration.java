...
public class GroupItemDecoration extends RecyclerView.ItemDecoration implements IGroupItemDecoration{
    ...
    private List<GroupItem> groupList = new ArrayList<>();//设置的分组列表
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if(isFirst){
            ...
            decorationCallback.setGroup(groupList);//获取设置的分组列表
            if(groupList.size()==0){//若没有设置分组，跳出（下同）
                return;
            }
        ...
        if(groups.get(position)!=null){
            outRect.top = groupViewHeight;//RecyclerView中该position对应的childView之前需要绘制groupView，则为其预留空间
        }
    }
