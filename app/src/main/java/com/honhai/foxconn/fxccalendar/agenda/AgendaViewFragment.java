...
public class AgendaViewFragment extends Fragment implements Rollback, RefreshData {
    ...
    private int getTodayPosition() {
        ...
        for (int i = 0; i < banners.size(); i++) {
            DayBanner banner = banners.get(i);
            c = banner.calendar.compareTo(calendar);
            if (c >= 0) {
                if (banner.position > mAgendaListRecyclerView.getChildAdapterPosition(mAgendaListRecyclerView.getChildAt(0)))
                    return banner.position;
                else if (i != 0)
                    return banners.get(i - 1).position;
                else
                    return banner.position;
            }
        }
