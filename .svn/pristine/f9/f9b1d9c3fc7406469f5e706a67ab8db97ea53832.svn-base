...
public class Data {
    ...
    private int eventType = EVENTS_UNCOMPLETED;
                        ...
                        switch (eventType) {
                            case EVENTS_COMPLETED:
                                for (int i = 0; i < groups.size(); i++) {
                                    if (event.getGroupId() == groups.get(i).getGroupId() && event.isComplete()) {
                                        events.add(event);
                                        break;
                                    }
                                }
            ...
            public void done(List<AVObject> aList, AVException e) {
                if (e == null) {
                    ...
                    for (AVObject object : avList) {
                        ...
                        group.setGroupIcon(object.getAVFile("groupIcon") == null ? null : object.getAVFile("groupIcon").getUrl());
            ...
            public void done(List<AVObject> aList, AVException e) {
                if (e == null) {
                    ...
                    for (AVObject object : avList) {
                        ...
                        users.add(user);
                        if (userInfo.getWorkId() != null && userInfo.getWorkId().equals(user.getWorkId())) {
                            userInfo = user;
                        }
                    }
    ...
    public void uploadGroupsIcon(String groupObjectId, String icon) {
        ...
        AVFile file = null;
        try {
            file = AVFile.withAbsoluteLocalPath(groupObjectId, icon);
    ...
    public void loadImageByGlide(ImageView imageView, String groupId) {
        String path = imageView.getContext().getFilesDir() + "/" + groupId + ".jpg";
        Drawable drawable = imageView.getContext().getDrawable(R.drawable.ic_people_black_24dp);
