package com.honhai.foxconn.fxccalendar.widget;
...
public class CustomDatePicker {
    ...
    public interface ResultHandler {
        void handle(TextView yearView, TextView timeView, String time);
    }
    ...
    private TextView mYearView, mTimeView;
    private TextView startYearText, startTime, endYearText, endTime;
    ...
    private void initDialog() {
        if (datePopupWindow == null) {
            window = ((AddEventActivity) context).getWindow();
            layout = View.inflate(context, R.layout.popup_window_time_picker_layout, null);
            int w = context.getResources().getDisplayMetrics().widthPixels;
            int h = context.getResources().getDisplayMetrics().heightPixels;
            datePopupWindow = new PopupWindow(layout, w - 80, (int) (h * .6f));
    ...
    private void initView() {
        ...
        year_pv = layout.findViewById(R.id.datePickerViewYear);
        month_pv = layout.findViewById(R.id.datePickerViewMonth);
        day_pv = layout.findViewById(R.id.datePickerViewDayOfMonth);
        hour_pv = layout.findViewById(R.id.datePickerViewHour);
        minute_pv = layout.findViewById(R.id.datePickerViewMinute);
        tv_cancle = layout.findViewById(R.id.timePickerCancel);
        tv_select = layout.findViewById(R.id.timePickerConfirm);
        startYearText = layout.findViewById(R.id.startYearText);
        startTime = layout.findViewById(R.id.startTimeText);
        endYearText = layout.findViewById(R.id.endYearText);
        endTime = layout.findViewById(R.id.endTimeText);
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CHINA);
                handler.handle(mYearView, mTimeView, sdf.format(selectedCalender.getTime()));
                datePopupWindow.dismiss();
            }
        });
    }
...
//        }, 100);
    ...
    public void show(TextView yearView, TextView timeView, String time) {
        if (canAccess) {
            if (isValidDate(time, "yyyy-MM-dd")) {
                if (startCalendar.getTime().getTime() < endCalendar.getTime().getTime()) {
                    ...
                    datePopupWindow.showAtLocation(window.getDecorView(), Gravity.CENTER, 0, 0);
                }
                mYearView = yearView;
                mTimeView = timeView;
            } else {
    ...
    public void setTimeText(String start, String end, boolean isStart) {
        String[] times = start.split(" ");
        startYearText.setText(times[0]);
        startTime.setText(times[1]);
        times = end.split(" ");
        endYearText.setText(times[0]);
        endTime.setText(times[1]);
        isStartSet = isStart;
    }

    private void setScrollTimeText() {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CHINA);
        if (isStartSet) {
            startTimeText = sdf.format(selectedCalender.getTime());
            String[] times = startTimeText.split(" ");
            startYearText.setText(times[0]);
            startTime.setText(times[1]);
        } else {
            endTimeText = sdf.format(selectedCalender.getTime());
            String[] times = endTimeText.split(" ");
            endYearText.setText(times[0]);
            endTime.setText(times[1]);
        }
